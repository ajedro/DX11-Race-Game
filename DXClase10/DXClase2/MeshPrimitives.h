...
public:
	...
	bool InitScene(ID3D11Device* d3d11Device, ID3D11DeviceContext* d3d11DevCon, LPCWSTR texturePath, LPCWSTR textureNormalPath, int ShaderType)
	{
		...
		LPCWSTR pixelShaderFile = L"pixelShader.ps";
		...
		case ShaderMultitextura:
			...
			break;
		case ShaderLines:
			pixelShaderFile = L"pixelShaderLines.ps";
			break;
		default:
			break;
		}
	...
	void UpdateScene_Obj(D3DXMATRIX World)
	{

		this->World = World*object_matrix;
	}
	...
	void DrawBitmap(Camara *camara){
		...
		D3DXMatrixTranslation(&TranslationM, .5, -.5, .8);
	...
	D3DXVECTOR3 get_obj_v() {
		return object_vector;
	}

	void set_obj_v(float X, float Y, float Z) {
	...
	void set_obj_v(D3DXVECTOR3 translation_v) {

		

		object_vector = translation_v;

		D3DXMatrixTranslation(&object_matrix, object_vector.x, object_vector.y, object_vector.z);


	}
	void X_set_obj_v(float X) {

		object_vector.x = X;
		
		D3DXMatrixTranslation(&object_matrix, X, object_vector.y, object_vector.z);


	}

	void Y_set_obj_v(float Y) {

		object_vector.y = Y;

		D3DXMatrixTranslation(&object_matrix, object_vector.x, Y, object_vector.z);


	}

	void Z_set_obj_v(float Z) {

		object_vector.z = Z;

		D3DXMatrixTranslation(&object_matrix, object_vector.x, object_vector.y,Z);


	}
	//Le dices hacia donde ir el objeto. calculando y haciendo un vector entre dos puntos.
	//mueve el objeto hacia cierta direccion
	void sum_obj_v(float X, float Y, float Z) {

		object_vector.x = object_vector.x + X;
		object_vector.y = object_vector.y + Y;
		object_vector.z = object_vector.z + Z;
