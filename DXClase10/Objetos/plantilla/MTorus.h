...
public:
	...
	MTorus(ID3D11Device *d3d11Device, ID3D11DeviceContext* d3d11DevCon, int Slices, int Stacks, float RadiusMajor, float RadiusMinor, LPCWSTR text, HWND hwnd)
	{
	...
	bool InitScene(HWND hwnd)
	{
		...
		for(int i = 0; i < Stacks; i++)
		{
			...
			for(int j = 0; j < Slices; j++)
			{
				...
				vertex[index].pos.x   = (RadiusMajor + RadiusMinor * cos(x*phix)) * cos(y*thetax);
				vertex[index].pos.y   = (RadiusMajor + RadiusMinor * cos(x*phix)) * sin(y*thetax);
				vertex[index].pos.z   =  RadiusMinor * sin(x*phix);
