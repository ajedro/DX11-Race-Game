...
public:
	...
	MCilinder(ID3D11Device *d3d11Device, ID3D11DeviceContext* d3d11DevCon, int Slices, float Alto, float Radius, LPCWSTR text, HWND hwnd)
	{
	...
	bool InitScene(HWND hwnd)
	{
		...
		SimpleVertex *vertex = new SimpleVertex[Slices * cantSectionsInCilinder];

		for(int i = 0; i < cantSectionsInCilinder; i++)
		{
		...
		vertexBufferDesc.ByteWidth = sizeof(SimpleVertex)* (Slices * cantSectionsInCilinder);
		...
		DWORD *indices = new DWORD[Slices * cantSectionsInCilinder * numVertInTwoTriangleFaces];
		index = 0;
		int indexBottom = Slices;

		for(int i=0; i < cantSectionsInCilinder - 1; i++)
		{
		...
		indexBufferDesc.ByteWidth = sizeof(DWORD)* Slices * cantSectionsInCilinder * numVertInTwoTriangleFaces;
	...
	void DrawScene(Camara *camara)
	{
		...
		d3d11DevCon->DrawIndexed(Slices * cantSectionsInCilinder * numVertInTwoTriangleFaces, 0, 0);
	}
